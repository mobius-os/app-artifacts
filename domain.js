...
async function publicKeys(){var t=token();if(!t)throw err('unavailable','Artifact storage public context is unavailable.');try{var response=await w.fetch('/api/published-sites/'+encodeURIComponent(t)+'/data',{cache:'no-store',credentials:'omit'});if(response.status===404)return [];if(!response.ok)throw err('request-failed','Artifact storage request failed ('+response.status+').');var body=await response.json();return body&&Array.isArray(body.keys)?body.keys:[]}catch(e){if(e&&e.code)throw e;throw err('network-error','Artifact storage request failed.') }}
